...
<AppLayout {breadcrumbs}>
    <div class="mx-auto w-full max-w-6xl space-y-6 px-4 py-6">
        <div class="space-y-1">
            ...
            <p class="text-sm text-muted-foreground">Overview of registered users and their activity.</p>
        </div>

        <section class="max-h-[calc(100vh-13rem)] overflow-y-auto rounded-lg border">
            <div class="sticky top-0 z-10 hidden grid-cols-[minmax(0,1.2fr)_minmax(0,1.5fr)_5rem_5rem_minmax(0,1fr)_10rem] gap-4 border-b bg-muted/95 px-4 py-3 text-xs font-medium text-muted-foreground backdrop-blur 2xl:grid">
                <span>Name</span>
                <span>Email</span>
                <span>Ads</span>
                <span>Images</span>
            ...
            {:else}
                {#each users as user (user.id)}
                    <article class="grid gap-3 border-b px-4 py-4 last:border-b-0 sm:grid-cols-2 2xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1.5fr)_5rem_5rem_minmax(0,1fr)_10rem] 2xl:items-center 2xl:gap-4">
                        <div class="min-w-0">
